...
class Displayer:
        ...
        for x in range(self._map.width):
            for y in range(self._map.height):
                ...
                if self._status.game_end or self._status.enemy.revealed((x, y)):
                    ...
                    # Show checkpoints A/B labels when discovered by enemy or when game ended
                    # Show large A/B only after enemy has visited them
                    if self._status.checkpoint_a == (x, y) and self._status.enemy_visited_a:
                        self._draw_badge(map_surf, rect, "A", (255, 215, 0))
                    if self._status.checkpoint_b == (x, y) and self._status.enemy_visited_b:
                        self._draw_badge(map_surf, rect, "B", (0, 255, 255))

                    # Show tunnel entry marker if discovered or after game end
                    if (x, y) in self._status.discovered_tunnel_entries or (self._status.game_end and (x, y) in self._status.tunnels):
                        self._draw_badge(map_surf, rect, "⟂", (186, 85, 211))
                else:
                    # Show black fog.
                    if (x, y) in path:
    ...
    def _draw_badge(self, surf: pg.Surface, rect: pg.Rect, text: str, color: tuple[int, int, int]) -> None:
        # Draw a filled circle with large text centered (bigger marker)
        radius = 14
        center = (rect.centerx, rect.top + 16)
