...
class Status:
    ...
    def __init__(self) -> None:
        ...
        # Checkpoints A and B that the enemy must visit before capture is allowed
        self.checkpoint_a: tuple[int, int] | None = None
        self.checkpoint_b: tuple[int, int] | None = None
        self.enemy_visited_a: bool = False
        self.enemy_visited_b: bool = False

        # Hidden tunnel mapping: entering a key position teleports to its value position
        self.tunnels: dict[tuple[int, int], tuple[int, int]] = {}

        # Discovered tunnel entries for rendering (populated after game end or upon stepping over)
        self.discovered_tunnel_entries: set[tuple[int, int]] = set()
    ...
    # Utility helpers for gameplay rules
    def set_checkpoints(self, a: tuple[int, int], b: tuple[int, int]) -> None:
        self.checkpoint_a = a
        self.checkpoint_b = b

    def set_tunnels(self, mapping: dict[tuple[int, int], tuple[int, int]]) -> None:
        self.tunnels = mapping

    def enemy_can_capture(self) -> bool:
        return self.enemy_visited_a and self.enemy_visited_b

    def enemy_target(self) -> tuple[int, int]:
        """
        Decide the current target for the enemy:
        1) If A not visited or B not visited, go to the closer of the remaining checkpoints
        2) Otherwise, target the agent position
        """
        assert self.enemy is not None and self.agent is not None
        remaining: list[tuple[int, int]] = []
        if self.checkpoint_a and not self.enemy_visited_a:
            remaining.append(self.checkpoint_a)
        if self.checkpoint_b and not self.enemy_visited_b:
            remaining.append(self.checkpoint_b)
        if len(remaining) == 0:
