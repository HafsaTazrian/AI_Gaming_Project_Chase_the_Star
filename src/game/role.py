...
class Role:
    ...
    def _try_move(self, pos: tuple[int, int]) -> bool:
        ...
        if occupy != gm.Occupy.NONE:
            ...
            self._pos = pos
            # Hidden tunnel teleportation (unknown to players)
            if self._pos in self._status.tunnels:
                # Mark entry discovered for later rendering
                self._status.discovered_tunnel_entries.add(self._pos)
                self._pos = self._status.tunnels[self._pos]
    ...
    def _load_strategies(self) -> None:
        ...
        for s, w in cfg.strategy_weights("agent").items():
            ...
            if s == sg.Random.name():
                ...
                self._strategies.append(sg.AStar(self))
            elif s == sg.Dijkstra.name():
                self._strategies.append(sg.Dijkstra(self))
            elif s == sg.BFS.name():
                self._strategies.append(sg.BFS(self))
            elif s == sg.Greedy.name():
                self._strategies.append(sg.Greedy(self))
            elif s == sg.JPS.name():
                self._strategies.append(sg.JPS(self))
            else:
...
class Enemy(Role):
    def __init__(self, status: 'gm.Status', map: 'gm.Map', pos: tuple[int, int]) -> None:
        super().__init__(status, map, pos)
        self._pathfinding_strategy = None
        self._load_strategies()

    @property
    def path(self) -> list[tuple[int, int]]:
        """
        Get the pathfinding path if it exists.
        """
        if not self._pathfinding_strategy:
            return []
        if hasattr(self._pathfinding_strategy, 'prev_path'):
            return self._pathfinding_strategy.prev_path
        return []
    ...
    def _load_strategies(self) -> None:
        ...
        for s, w in cfg.strategy_weights("enemy").items():
            ...
            if s == sg.Random.name():
                ...
                strategy = sg.AStar(self)
                self._pathfinding_strategy = strategy
                self._strategies.append(strategy)
            elif s == sg.Dijkstra.name():
                strategy = sg.Dijkstra(self)
                self._pathfinding_strategy = strategy
                self._strategies.append(strategy)
            elif s == sg.BFS.name():
                strategy = sg.BFS(self)
                self._pathfinding_strategy = strategy
                self._strategies.append(strategy)
            elif s == sg.Greedy.name():
                strategy = sg.Greedy(self)
                self._pathfinding_strategy = strategy
                self._strategies.append(strategy)
            elif s == sg.JPS.name():
                strategy = sg.JPS(self)
                self._pathfinding_strategy = strategy
                self._strategies.append(strategy)
            else:
                raise ValueError("Invalid action strategy.")
